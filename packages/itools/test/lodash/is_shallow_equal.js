...
describe('isShallowEqual', () => {
  ...
  it('should compare "arrays"', () => {
    ...
    expect(itools.isShallowEqual([{ a: 1 }, { a: 1 }], [{ a: 1 }, { a: 1 }])).toBe(true);
  ...
  it('should compare "objects"', () => {
    ...
    expect(
      itools.isShallowEqual({ a: { b: [1, 2, { c: 1, d: { e: 1 } }] } }, { a: { b: [1, 2, { c: 1, d: { e: 1 } }] } })
    ).toBe(true);
    expect(itools.isShallowEqual({ a: 1 }, { a: 2 })).toBe(false);
    expect(itools.isShallowEqual({ a: [1] }, { a: [2] })).toBe(false);
    expect(itools.isShallowEqual({ a: { b: 1 } }, { a: { b: 2 } })).toBe(false);
    expect(
      itools.isShallowEqual({ a: { b: [1, 2, { c: 1, d: { e: 1 } }] } }, { a: { b: [1, 2, { c: 1, d: { e: 2 } }] } })
    ).toBe(false);
  });
  ...
  it('should compare multiple (> 2) values', () => {
    ...
    expect(itools.isShallowEqual([1, 2], [1, 2, 3], [1, 2], [1, 2])).toBe(false);
  });
