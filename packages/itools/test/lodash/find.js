...
describe('find', function () {
   ...
   */
  it('itools.find should support shortcut fusion', function () {
    let isFind = itools['find'] === 'find';
    let findCount = 0;
    let mapCount = 0;
    ...
    let actual = _(array).map(iteratee)['find'](predicate);
  ...
  test('returns base case', () => {
    ...
    const findObj = itools.find(users, function (o) {
      return o.age < 40;
    });

    expect(itools.find(users, findObj)).toEqual(barneyObject);
    expect(itools.find(users, { age: 1, active: true })).toEqual(pebblesObject);
    expect(itools.find(users, { active: false })).toEqual({
      active: false,
      age: 40,
      user: 'fred'
    });
