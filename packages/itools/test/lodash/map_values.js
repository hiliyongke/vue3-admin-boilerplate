...
describe('mapValues', function () {
  test('applies predicate using value argument', () => {
    ...
    let result1 = itools.mapValues(obj1, function (value) {
      return value + 1;
    });
    expect(result1).toStrictEqual({ a: 4, b: 6, c: 10 });

    let obj2 = { a: 3, b: 0, c: null };
    let result2 = itools.mapValues(obj2, function (value) {
  ...
  test('applies predicate using key argument', function () {
    let obj1 = { a: 3, b: 5, c: 9 };
    let result1 = itools.mapValues(obj1, function (value, key) {
      return key;
    });
    expect(result1).toStrictEqual({ a: 'a', b: 'b', c: 'c' });

    let obj2 = [1, 2, 3];
    let result2 = itools.mapValues(obj2, function (value, key) {
  ...
  test('applies predicate using all arguments', function () {
    let obj1 = { a: 3, b: 5, c: 9 };
    let result1 = itools.mapValues(obj1, function (value, key, obj) {
