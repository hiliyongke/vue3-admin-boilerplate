  ...
  describe('base case', () => {
    describe('equal', () => {
      test.each([
        ...
        expect(itools.objectDiff(value, value)).toEqual({});
      });
    });
    ...
    describe('not equal and not object', () => {
      test.each([
        ...
        expect(itools.objectDiff(lhs, rhs)).toEqual(rhs);
      });
    });
  });
  ...
  describe('recursive case', () => {
    describe('object', () => {
      ...
      test('returns subset of right hand side value when a key value has been deleted', () => {
        expect(itools.objectDiff({ a: { b: 1 }, c: 2, d: { e: 100 } }, { a: { b: 1 }, c: 2, d: {} })).toEqual({
          d: { e: undefined },
        });
      });

      test('returns subset of right hand side value when a key value has been added', () => {
        expect(itools.objectDiff({ a: 1 }, { a: 1, b: 2 })).toEqual({ b: 2 });
      });

      test('returns keys as undefined when deleted from right hand side', () => {
        expect(itools.objectDiff({ a: 1, b: { c: 2 } }, { a: 1 })).toEqual({
          b: undefined,
        });
      });
    });
    ...
    describe('arrays', () => {
      ...
      test('returns subset of right hand side array as object of indices to value when right hand side array has additions', () => {
        expect(itools.objectDiff([1, 2, 3], [1, 2, 3, 9])).toEqual({ 3: 9 });
      });
    });
    ...
    describe('date', () => {
      ...
      test('returns empty object when dates are equal', () => {
        expect(itools.objectDiff(new Date('2016'), new Date('2016'))).toEqual({});
      });

      test('returns right hand side date when updated', () => {
        expect(itools.objectDiff({ date: lhs }, { date: rhs })).toEqual({
          date: rhs,
        });
        expect(itools.objectDiff([lhs], [rhs])).toEqual({ 0: rhs });
      });

      test('returns undefined when date deleted', () => {
        expect(itools.objectDiff({ date: lhs }, {})).toEqual({
          date: undefined,
        });
        expect(itools.objectDiff([lhs], [])).toEqual({ 0: undefined });
      });

      test('returns right hand side when date is added', () => {
        expect(itools.objectDiff({}, { date: rhs })).toEqual({ date: rhs });
        expect(itools.objectDiff([], [rhs])).toEqual({ 0: rhs });
      });
    });
    ...
    describe('object create null', () => {
      test('returns right hand side value when given objects are different', () => {
        ...
        rhs.a = 2;
        expect(itools.objectDiff(lhs, rhs)).toEqual({ a: 2 });
      });
      ...
      test('returns subset of right hand side value when sibling objects differ', () => {
        ...
        rhs.a = { b: 1 };
        rhs.c = 3;
        expect(itools.objectDiff(lhs, rhs)).toEqual({ c: 3 });
      });
      ...
      test('returns subset of right hand side value when nested values differ', () => {
        ...
        const rhs = Object.create(null);
        rhs.a = { b: 1, c: 3 };
        expect(itools.objectDiff(lhs, rhs)).toEqual({ a: { c: 3 } });
      });
      ...
      test('returns subset of right hand side value when nested values differ at multiple paths', () => {
        ...
        rhs.a = { b: 99 };
        rhs.c = 3;
        expect(itools.objectDiff(lhs, rhs)).toEqual({ a: { b: 99 }, c: 3 });
      });
      ...
      test('returns subset of right hand side value when a key value has been deleted', () => {
        ...
        const rhs = Object.create(null);
        rhs.a = { b: 1 };
        expect(itools.objectDiff(lhs, rhs)).toEqual({ c: undefined });
      });
      ...
      test('returns subset of right hand side value when a key value has been added', () => {
        ...
        rhs.a = 1;
        rhs.b = 2;
        expect(itools.objectDiff(lhs, rhs)).toEqual({ b: 2 });
      });
    });
  });
});
